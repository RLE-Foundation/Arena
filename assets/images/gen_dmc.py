...
for env_name in envs:
    env = make(env_name, 3, 2, 1)
    ts = env.reset()
    ts = env.step(np.random.rand(env.action_spec().shape[0], ))
    
    img = ts.observation[:3, :, :].transpose(1, 2, 0)
    img = Image.fromarray(img)
    img.save(f'assets/images/dmc/{env_name}.png')
    print(env_name, env.action_spec())
    env.close()